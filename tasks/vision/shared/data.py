...
def apply_transform(pos: torch.Tensor, feats: torch.Tensor, rotate: bool = False, shear: bool = False, mirror: bool = False, shear_range: float = 0.8) -> Tuple[torch.Tensor, torch.Tensor]:
    """Apply random transformations to positions."""
    device = pos.device
    transform_matrix = torch.eye(2, device=device, dtype=torch.float32)

    if mirror and torch.rand(()) > 0.5:
        mirror_matrix = torch.tensor([[-1.0, 0.0], [0.0, 1.0]], device=device, dtype=torch.float32)
        transform_matrix = transform_matrix @ mirror_matrix

    if shear and torch.rand(()) > 0.5:
        shear_val = (torch.rand(()) - 0.5) * shear_range
        shear_matrix = torch.tensor([[1.0, shear_val], [0.0, 1.0]], device=device, dtype=torch.float32)
        transform_matrix = transform_matrix @ shear_matrix

    if rotate:
        angle = (torch.rand(()) * 2 - 1) * (torch.pi / 12)
        cos_theta = torch.cos(angle)
        sin_theta = torch.sin(angle)
        rotate_matrix = torch.tensor([[cos_theta, -sin_theta], [sin_theta, cos_theta]], device=device, dtype=torch.float32)
        transform_matrix = transform_matrix @ rotate_matrix

    pos_float = pos.float()
    centered = pos_float - pos_float.mean(dim=0, keepdim=True)
    transformed = centered @ transform_matrix
    transformed = transformed - transformed.min(dim=0, keepdim=True).values

    return transformed.int(), feats.clone()


def copy_and_transform_points(pos: torch.Tensor, feats: torch.Tensor, num_target: int, rotate: bool = False, shear: bool = False, mirror: bool = False, shear_range: float = 0.8) -> Tuple[torch.Tensor, torch.Tensor]:
    """Augment points by tiling along bounding box directions."""
    import math
    import numpy as np

    current = pos.size(0)
    if current >= num_target:
        return pos, feats

    device = pos.device
    dtype = pos.dtype

    # Calculate number of copies needed (each iteration creates 4x points)
    num_copies = math.ceil(math.log(num_target / current, 4))

    min_vals = pos.min(dim=0).values.float()
    max_vals = pos.max(dim=0).values.float()
    bounding_box_size = max_vals - min_vals

    for _ in range(num_copies):
        translation_vector_x = torch.tensor([bounding_box_size[0], 0], dtype=torch.float32, device=device)
        translation_vector_y = torch.tensor([0, bounding_box_size[1]], dtype=torch.float32, device=device)
        translation_vector_diag = torch.tensor([bounding_box_size[0], bounding_box_size[1]], dtype=torch.float32, device=device)

        # Apply translations
        new_points_x = pos.float() + translation_vector_x
        new_points_y = pos.float() + translation_vector_y
        new_points_diag = pos.float() + translation_vector_diag

        # Optionally apply random transforms
        pos_1, feat_1 = apply_transform(new_points_x.int(), feats, rotate=rotate, shear=shear, mirror=mirror, shear_range=shear_range)
        pos_2, feat_2 = apply_transform(new_points_y.int(), feats, rotate=rotate, shear=shear, mirror=mirror, shear_range=shear_range)
        pos_3, feat_3 = apply_transform(new_points_diag.int(), feats, rotate=rotate, shear=shear, mirror=mirror, shear_range=shear_range)

        # Concatenate new points
        pos = torch.cat([pos, pos_1, pos_2, pos_3], dim=0)
        feats = torch.cat([feats, feat_1, feat_2, feat_3], dim=0)

    # Remove duplicate positions (keeps first occurrence)
    pos_np = pos.cpu().numpy()
    _, unique_indices = np.unique(pos_np, axis=0, return_index=True)
    unique_indices = torch.from_numpy(unique_indices).to(device)
